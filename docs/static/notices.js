...
// GitHub Gist를 중앙 데이터 저장소로 사용 (gist-sync.js 사용)
...
// GitHub Gist에서 공지 목록 가져오기 (gist-sync.js 사용)
async function fetchNoticesFromGist() {
    if (window.GistSync) {
        return await window.GistSync.fetchFromGist(
            window.GistSync.GIST_CONFIG.FILES.NOTICES,
            'eventbet_notices'
        );
    } else {
        console.warn('[NOTICE] GistSync not loaded, using localStorage');
